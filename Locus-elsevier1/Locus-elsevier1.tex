...
colour magnitude values between 15 and 20. Care is taken to ensure that
all 7112 stars in the 3556 pairs are unique. The SQL queries used to
...
The histogram of the residuals from this fit on the validation data is
shown in figure \ref{fig:histogram-residuals}. Figure \ref{fig:obs-pred}
shows the resulting values of observed logit correlation values against
predicted logit correlation values. Figure \ref{fig:residuals-pred}
shows the resulting values of the residuals, the observations minus the
predicted values, against predicted logit correlation values. Figure
